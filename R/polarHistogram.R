#' @title Polar Histogram v. 21022012
#' @description 
#' Builds many histograms arranged around a circle 
#'
#' @param df a data frame containing the data
#' @param binSize width of the bin. Should probably be left as 1, 
#'        as other parameters are relative to it.
#' @param spaceItem space between bins
#' @param spaceFamily space between families
#' @param innerRadius radius of inner circle
#' @param outerRadius radius of outer circle. Should probably be left 
...
#' @param direction whether the increasing count goes from or to the center.
#' @param familyLabels logical. Whether to show family names
#'
#' @return a ggplot object
#' 
#' @details 
#' The data frame is expected to have at least four columns: family, item, score 
#' and value. The three first columns are categorical variables, the fourth column 
#' contains non-negative values. Each bar represents the proportion of scores for 
#' one item. Items can be grouped by families. The resulting graph can be busy and 
#' might be better off saved as a pdf, with ggsave.
#'
#' @references 
#' Kettunen, J. et al. (2012) Genome-wide association study identifies multiple loci 
#' influencing human serum metabolite levels. Nat Genet advance online 
#' publication. URL http://dx.doi.org/10.1038/ng.1073.
#' CC licence: http://creativecommons.org/licenses/by-nc-sa/3.0/ 
#'
#' @author Christophe Ladroue, modifications Jeffrey S. Evans  <jeffrey_evans@@tnc.org>
#'
#' @examples
#' library(plyr)
#' library(ggplot2)
...
#' nValues = 3
#' 
#'   df <- data.frame(family=rep(randomName(nFamily),nItemPerFamily),
#'                  item=randomName(sum(nItemPerFamily),2))
#'   df <- cbind(df,as.data.frame(matrix(runif(nrow(df)*nValues),
#'               nrow=nrow(df),ncol=nValues)))
#'   # from wide to long			  
#'   df <- melt(df,c("family","item"),variable_name="score") 
#'   
#'   p <- polarHistogram(df,familyLabel=FALSE)
#'     print(p)
#'
#' @export
polarHistogram <- function(df, binSize=1, spaceItem=0.2,
                           spaceFamily=1.2, innerRadius=0.3,
                           outerRadius=1, guides=c(10,20,40,80),
